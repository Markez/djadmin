...
// 
    ...
    public function create_banner($data){
        ...
        $sql=$this->db->insert('banners',$data);
    ...
    public function create_story($data){
        ...
        $sql=$this->db->insert('stories',$data);
    ...
    public function updateStaffState(){
        ...
        // 
    public function updateBannerState(){
        $id = $this->input->post('id');
        $state = $this->input->post('state');

        $q=$this->db->query("SELECT * FROM banners WHERE bannerId='".$id."'");
        if($q->num_rows()<1)
        {
            echo "<script>alert('Internal Server Error Occured. Please Contact Technical Team');</script>";
        }
        ...
        {
            $query=$this->db->query('UPDATE banners SET status="'.$state.'" WHERE bannerId="'.$id.'"');
            if($query)
            {
                return True;
            }
    ...
    public function updateEmployerState(){
        ...
        // 
    public function updateStoryState(){
        $id = $this->input->post('id');
        $state = $this->input->post('state');

        $q=$this->db->query("SELECT * FROM stories WHERE storyId='".$id."'");
        if($q->num_rows()<1)
        {
            echo "<script>alert('Internal Server Error Occured. Please Contact Technical Team');</script>";
        }
        ...
        {
            $query=$this->db->query('UPDATE stories SET status="'.$state.'" WHERE storyId="'.$id.'"');
            if($query)
            {
                return True;
            }
    ...
    public function updateBanner($data){
        // verification and account creation
        $query=$this->db->query("SELECT * FROM banners WHERE bannerId='".$data['id']."'");
        if($query->num_rows()==1)
        {
            $date = date('Y-m-d H:i:s');
            // data prep
            ...
            $this->db->where('bannerId',$data['id']);
            $sql=$this->db->update('banners',$dat);
    ...
    public function updateStory($data){
        // verification and account creation
        $query=$this->db->query("SELECT * FROM stories WHERE storyId='".$data['id']."'");
        if($query->num_rows()==1)
        {
            $date = date('Y-m-d H:i:s');
            // data prep
            ...
            $this->db->where('storyId',$data['id']);
            $sql=$this->db->update('stories',$dat);
